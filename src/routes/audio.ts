...
export const transcriptionHandler = async (
	request: Request,
	env: Env,
): Promise<Response> => {
	...
	try {
		...
		const resp = (await env.AI.run(model as keyof AiModels, input)) as {
			text: string;
		};
...
export const translationHandler = async (
	...
	try {
		...
		const languageIdResp = (await env.AI.run(
			"@cf/meta/llama-2-7b-chat-int8" as keyof AiModels,
			{
				messages: [
					{
						role: "user",
		...
		const translationResp = (await env.AI.run(
